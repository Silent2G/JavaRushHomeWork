...
public class Solution
{
    ...
    public static void colorLight(double time) {
        int multiplier =(int)(time/5);
        int startTime = multiplier * 5;
        double lightNow = time - startTime;

        if(lightNow <= 2.999) {
            System.out.println("зеленый");
        }
        if(lightNow >= 3 && lightNow <=3.999) {
            System.out.println("желтый");
        }
        if(lightNow >= 4 && lightNow <= 4.999) {
