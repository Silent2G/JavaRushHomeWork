    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                String one = (String) stock1.get("name");
                String two = (String) stock2.get("name");
                Date date1 = (Date) stock1.get("date");
                Date date2 = (Date) stock2.get("date");
                SimpleDateFormat df = new SimpleDateFormat("yyyyMMdd");
                int compareNames = one.compareTo(two);
                int compareDates = df.format(date2).compareTo(df.format(date1));
                if (compareNames != 0) return compareNames;
                else if (compareDates != 0) return compareDates;
                else {
                    double number1 = (double) stock1.get("last") - (double) stock1.get("open");
                    double number2 = (double) stock2.get("last") - (double) stock2.get("open");
                    return Double.compare(number2, number1);
                }
            }
        });
    }
